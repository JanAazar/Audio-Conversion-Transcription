...
from hume.models.config import Prosody
from hume.models.model_type import Models
from hume.models.inference import InferenceBaseRequest
from speechmatics.batch import BatchClient
...
def get_audio_data_hash(audio_file_content):
    return hash(audio_file_content)

@st.cache_data(show_spinner="🔄 Converting to mono...")
def convert_to_mono(audio_file_path, target_sr=48000):
    audio_data, sample_rate = librosa.load(audio_file_path, sr=target_sr, mono=False)
...
def process_audio(first_file_path, second_file_path, folder_name):
    base_folder = "Recording"
    output_folder = os.path.join(base_folder, folder_name)
    os.makedirs(output_folder, exist_ok=True)
    
    st.info("🔄 Converting and aligning files...")
    first_data, first_sr = convert_to_mono(first_file_path)
    second_data, second_sr = convert_to_mono(second_file_path)
...
def split_stereo_conversation(stereo_file_path, folder_name):
    base_folder = "Recording"
    output_folder = os.path.join(base_folder, folder_name)
    os.makedirs(output_folder, exist_ok=True)
    
    st.info("🔄 Loading stereo audio file...")
    audio_data, sample_rate = librosa.load(stereo_file_path, sr=None, mono=False)
        ...
        if 'results' in response_json and 'channels' in response_json['results']:
            for channel in response_json['results']['channels']:
                if 'alternatives' in channel and len(channel['alternatives']) > 0:
                    ...
                    if paragraphs_data:
                        paragraphs_list = paragraphs_data.get('paragraphs', []) if isinstance(paragraphs_data, dict) else paragraphs_data
...
def speechmatics_analysis(audio_path, output_folder, language="es"):
    try:
        api_key = os.getenv("SPEECHMATICS_API_KEY")
        if not api_key:
            st.error("❌ SPEECHMATICS_API_KEY not found in environment variables.")
            return None
        ...
        with BatchClient(settings) as client:
            ...
            summary_file = os.path.join(output_folder, "summary_speechmatics.txt")
            with open(summary_file, 'w', encoding='utf-8') as f:
                f.write(summary)
            
            metadata_file = os.path.join(output_folder, "metadata_speechmatics.txt")
            with open(metadata_file, 'w', encoding='utf-8') as f:
                f.write("=== AUDIO EVENT SUMMARY ===\n\n")
                f.write(json.dumps(audio_event_summary, indent=2, ensure_ascii=False))
                f.write("\n\n=== DETECTED AUDIO EVENTS ===\n\n")
...
async def run_hume_analysis_async(audio_path, job_id=None):
    ...
    if not job_id:
        ...
        return {'job_id': job_id, 'status': 'SUBMITTED', 'predictions': None}
    ...
    if job_predictions and len(job_predictions) > 0:
        pred_str = str(job_predictions[0])
        if 'results=InferenceResults(predictions=[]' not in pred_str:
            return {'job_id': job_id, 'status': 'COMPLETED', 'predictions': job_predictions}
    
    job_details = await client.expression_measurement.batch.get_job_details(id=job_id)
    status_match = re.search(r"status='([^']+)'", str(job_details))
    status = status_match.group(1) if status_match else "UNKNOWN"
    
    if status == "FAILED":
        raise Exception(f"Job failed: {json.dumps(job_details, indent=2, default=str)}")
    
    return {'job_id': job_id, 'status': status, 'predictions': None}

def hume_emotion_analysis(audio_path, output_folder, job_id=None):
    
    if job_id and 'hume_job_status' in st.session_state and st.session_state.hume_job_status == 'COMPLETED':
        return st.session_state.hume_result
        
    try:
        if not get_hume_client():
        ...
        if not job_id:
            with st.spinner("🔄 Submitting job to Hume API..."):
                result = loop.run_until_complete(run_hume_analysis_async(audio_path))
            
            st.session_state.hume_job_id = result['job_id']
            st.session_state.hume_job_status = 'SUBMITTED'
            st.toast(f"✅ Job submitted! ID: {result['job_id']}. Waiting for completion...", icon="⏳")
            
            # Rerun to show status and wait
            st.rerun()

        elif job_id:
            with st.spinner(f"🔄 Waiting for predictions (Status: {st.session_state.hume_job_status})..."):
                result = loop.run_until_complete(run_hume_analysis_async(audio_path, job_id=job_id))
            
            st.session_state.hume_job_status = result['status']
            ...
            if result['status'] == 'COMPLETED':
                ...
                st.session_state.hume_result = hume_result
                return hume_result
            
            elif result['status'] != 'FAILED':
                st.info(f"Status: {result['status']}. Still processing...")
                time.sleep(5)
                st.rerun()
        ...
        st.session_state.hume_job_status = 'FAILED'
        st.error(f"❌ Hume emotion analysis failed: {e}")
        traceback.print_exc()
        return None
    finally:
...
if st.session_state.current_page == 'annotation':
    ...
    if conversation_audio and transcript_file:
        ...
        if transcript_entries:
            ...
            tmp_audio_path = save_uploaded_file(conversation_audio, f".{conversation_audio.name.split('.')[-1]}")
    ...
    if just_merge_button:
        if not first_file or not second_file:
            ...
            try:
                tmp_first_path = save_uploaded_file(first_file, f"_{first_file.name}")
                tmp_second_path = save_uploaded_file(second_file, f"_{second_file.name}")
    ...
    if split_button:
        if not split_conversation_file:
            ...
            try:
                tmp_audio_path = save_uploaded_file(split_conversation_file, f"_{split_conversation_file.name}")
    ...
    if transcribe_button:
        if not conversation_file:
            ...
            try:
                ...
                tmp_audio_path = save_uploaded_file(conversation_file, f"_{conversation_file.name}")
    ...
    if final_button:
        # Reset Hume state
        st.session_state.pop('hume_job_id', None)
        st.session_state.pop('hume_job_status', None)
        st.session_state.pop('hume_result', None)
        ...
        if not final_audio_file:
            ...
            try:
                ...
                tmp_audio_path = save_uploaded_file(final_audio_file, f"_{final_audio_file.name}")
                conversation_path = os.path.join(output_folder, "conversation.wav")
                ...
                st.markdown("---")
                st.markdown("### 😊 Step 2: Emotion Analysis")
                st.session_state.audio_for_hume = conversation_path
                st.session_state.output_folder_for_hume = output_folder
                # Fall through to the Hume processing block below

            except Exception as e:
                st.error(f"❌ Error creating final transcript: {str(e)}")
                st.exception(e)
    
    # Hume Polling Logic: This runs on every Streamlit rerun if a job is in progress
    if 'hume_job_id' in st.session_state and st.session_state.hume_job_status in ['SUBMITTED', 'PENDING', 'RUNNING']:
        hume_result = hume_emotion_analysis(st.session_state.audio_for_hume, 
                                            st.session_state.output_folder_for_hume, 
                                            job_id=st.session_state.hume_job_id)
        
    # Final merging step after Hume is completed
    if 'hume_job_status' in st.session_state and st.session_state.hume_job_status == 'COMPLETED':
        st.success("✅ Hume emotion analysis completed!")
        
        output_folder = st.session_state.output_folder_for_hume
        hume_result = st.session_state.hume_result
